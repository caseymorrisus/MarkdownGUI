...
function watchDirectory (bool) {
	if (bool) {
		...
		gaze = null;
		gaze = new Gaze('*.md', {cwd: options.watch.directory}, function (err, watcher) {
			this.on('changed', function (filepath) {
				console.log(filepath + ' was changed');
				var dest = getDestination(filepath);
				start = +new Date();
				readMarkdownFile(filepath, dest, compileMarkdown);
			});
...
holder.ondrop = function (e) {
	...
	if ( files.length === 1 ) {
		...
		if ( extension === '.md' ) {
			readMarkdownFile(file.path, dest, compileMarkdown);
		} else {
			changeInfoText("File was not a Markdown file.");
		};
	} else if ( files.length > 1 ) {
		var markdownFiles = [];
		_.forEach(files, function(n, key) {
			if ( path.extname(files[key].path) === '.md' ) {
				markdownFiles.push(files[key].path);
			}
		});
		readManyMarkdownFiles(markdownFiles, dest, compileMarkdown);
	};
	return false;
};
...
function compileMarkdown (markdownStr, fileName, dest , many) {
	var html = markdown.toHTML(markdownStr);

	if (options.html.minify) {
		console.log("MINIFY THE HTML");
		html = minify(html, {
			removeComments: true,
...
function readMarkdownFile (src, dest, callback) {
	var fileName = path.basename(src, '.md');
...
function readManyMarkdownFiles (array, dest, callback) {
